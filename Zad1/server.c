...
main ()
{
    ...
    while (1)
    {
        char ch[4];
        message m;
        int message_len = get_message_size(&m);
        ...
        if (fork () == 0)
        {
            ...
            deserialize_message(&m, buffor);
            double number = get_number(&m);
            int rq_id = get_rq_id(&m);
            printf("The id is: %d. The number is: %f", rq_id, number);


            write (client_sockfd, &ch, 4 * sizeof(char));
            close (client_sockfd);
            exit (0);
        }
